import { NextRequest, NextResponse } from 'next/server'
import { createServerSupabaseClient, createServiceRoleSupabaseClient } from '@/lib/supabase/server'
import { writeAuditLog } from '@/lib/audit'

async function requireActiveUser() {
  const supabase = await createServerSupabaseClient()
  const {
    data: { user },
  } = await supabase.auth.getUser()

  if (!user) return { error: NextResponse.json({ error: 'Oturum bulunamadı' }, { status: 401 }) }

  const { data: profile } = await supabase.from('profiles').select('id, is_active').eq('id', user.id).maybeSingle()
  if (!profile?.is_active) return { error: NextResponse.json({ error: 'Aktif kullanıcı bulunamadı' }, { status: 403 }) }

  return { user }
}

export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const auth = await requireActiveUser()
  if ('error' in auth) return auth.error

  const body = await request.json().catch(() => ({}))
  const isCompleted = Boolean(body.isCompleted)
  const service = createServiceRoleSupabaseClient()

  const { data: oldEvent, error: oldError } = await service.from('calendar_events').select('*').eq('id', id).maybeSingle()
  if (oldError) return NextResponse.json({ error: oldError.message }, { status: 500 })
  if (!oldEvent) return NextResponse.json({ error: 'Ajanda kaydı bulunamadı' }, { status: 404 })

  const { data, error } = await service
    .from('calendar_events')
    .update({ is_completed: isCompleted, completed_at: isCompleted ? new Date().toISOString() : null })
    .eq('id', id)
    .select('*')
    .single()

  if (error) return NextResponse.json({ error: error.message || 'Görev durumu güncellenemedi' }, { status: 500 })

  await writeAuditLog(service, {
    actorId: auth.user.id,
    action: isCompleted ? 'calendar_event.completed' : 'calendar_event.reopened',
    entityType: 'calendar_event',
    entityId: id,
    oldValues: oldEvent,
    newValues: data,
  })

  return NextResponse.json({ event: data })
}
