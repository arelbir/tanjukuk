...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: oldEvent, error: oldError } = await service.from('calendar_events').select('*').eq('id', id).maybeSingle()
  if (oldError) return NextResponse.json({ error: oldError.message }, { status: 500 })
  if (!oldEvent) return NextResponse.json({ error: 'Ajanda kaydı bulunamadı' }, { status: 404 })

  const { data, error } = await service.from('calendar_events').update(toCalendarEventPayload(parsed.data)).eq('id', id).select('*').single()
  if (error) return NextResponse.json({ error: error.message || 'Ajanda kaydı güncellenemedi' }, { status: 500 })

  if (parsed.data.event_type === 'hearing') {
    const { error: hearingError } = await service.from('hearing_details').upsert(toHearingDetailPayload(parsed.data, id))
