...
export async function POST(request: NextRequest) {
  ...
  if (error) return NextResponse.json({ error: error.message || 'İcra dosyası oluşturulamadı' }, { status: 500 })

  await writeAuditLog(service, {
    actorId: auth.user.id,
    action: 'enforcement_file.created',
    entityType: 'enforcement_file',
    entityId: data.id,
    newValues: data,
  })

  return NextResponse.json({ enforcementFile: data }, { status: 201 })
}
