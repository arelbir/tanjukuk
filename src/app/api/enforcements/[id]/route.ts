...
import { enforcementFileFormSchema } from '@/features/enforcements/schemas'
import { toEnforcementFilePayload } from '@/features/enforcements/repository'
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = enforcementFileFormSchema.safeParse(body)
  ...
  const { data, error } = await service.from('enforcement_files').update(toEnforcementFilePayload(parsed.data)).eq('id', id).select('*').single()
  if (error) return NextResponse.json({ error: error.message || 'İcra dosyası güncellenemedi' }, { status: 500 })

  await writeAuditLog(service, {
    actorId: auth.user.id,
    action: 'enforcement_file.updated',
