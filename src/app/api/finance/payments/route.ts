...
import { paymentFormSchema } from '@/features/finance/schemas'
import { toPaymentPayload } from '@/features/finance/repository'
...
async function requireFinanceUser() {
  ...
  if (!['admin', 'assistant', 'finance'].includes(profile.role)) return { error: NextResponse.json({ error: 'Tahsilat oluşturma yetkiniz yok' }, { status: 403 }) }
...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await service
    .from('payments')
    .insert({ ...toPaymentPayload(parsed.data), created_by: auth.user.id })
    .select('*')
    .single()

  if (error) return NextResponse.json({ error: error.message || 'Tahsilat oluşturulamadı' }, { status: 500 })
