...
import { receivableFormSchema } from '@/features/finance/schemas'
import { toReceivablePayload } from '@/features/finance/repository'
...
async function requireFinanceUser() {
  ...
  if (!['admin', 'assistant', 'finance'].includes(profile.role)) return { error: NextResponse.json({ error: 'Finans kaydı oluşturma yetkiniz yok' }, { status: 403 }) }
...
export async function POST(request: NextRequest) {
  ...
  const parsed = receivableFormSchema.safeParse(body)
  if (!parsed.success) return NextResponse.json({ error: getFirstZodError(parsed.error) }, { status: 400 })

  const service = createServiceRoleSupabaseClient()
  const { data, error } = await service
    .from('receivables')
    .insert({ ...toReceivablePayload(parsed.data), created_by: auth.user.id })
    .select('*')
    .single()

  if (error) return NextResponse.json({ error: error.message || 'Beklenen ödeme oluşturulamadı' }, { status: 500 })
